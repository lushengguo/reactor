...
class TcpServer : private noncopyable
{
  public:
    ...
    TcpServer(EventLoop *loop, const INetAddr &addr, std::string_view server_name) : endpoint_(addr), name_(server_name), started_(false), loop_(loop)
    {
    }
    ...
    void set_onConnectionCallback(EventCallback &&cb) { onConnectionCallback_ = std::move(cb); }

    void set_onMessageCallback(const MessageFunc &cb) { onMessageCallback_ = cb; }

    void set_onMessageCallback(MessageFunc &&cb) { onMessageCallback_ = std::move(cb); }
