...
class RedisClient : RedisProtocol
{
  public:
    ...
    void start()
    {
        client_.set_onConnectionCallback(std::bind(&RedisClient::onConnection, this, _1));
        client_.set_onMessageCallback(std::bind(&RedisClient::onMessage, this, _1, _2, _3));
        client_.start();
    }
  ...
  private:
    ...
    void onMessage(TcpConnectionPtr conn, Buffer &buffer, MicroTimeStamp receive_timestamp)
    {
        size_t len = 0;
        response_ = "";
        while ((len = parse_response(std::string(buffer.read_all_as_string()), response_)) != 0)
        {
            buffer.retrive(len);
            signal();
        }
...
int main(int argc, char **argv)
{
    ...
    client.start();
    // test function
    std::function<void(std::string, std::string)> TEST = [&](std::string input, std::string expect_response) {
        printf("input:[%s] expect-response:[%s] ", replace_all(input, "\r\n", "").c_str(), expect_response.c_str());
